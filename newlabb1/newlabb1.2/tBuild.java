...
*/
      ...
      //ArrayList<String> TestPositionList = new ArrayList<String>();
...
*/
      ...
      //hm.put(String.valueOf(hashWord(current[0])),String.valueOf(indexCounter)); //Add the first word and the correct position to the hashMap
...
try{
      while(in != null){
        ...
        //System.out.println("inde);xCounter: "+ indexCounter);


        //firstThreeNext = next[0].substring(0,Math.min(next[0].length(),3)); //Get first three letters of the word
        //firstThreeCurrent = current[0].substrcurrent[0]ing(0,Math.min(current[0].length(),3)); //Get first three letters of the word
...
/*
        if(hashWord(next[0]) != hashWord(current[0])){ //kollar om ord är annorlunda från den andra, isåfall lägg in i hashtabell

          hm.put(String.valueOf(hashWord(next[0])),String.valueOf(indexCounter)); //Lägger till unika ord
...
*/
    ...
    //System.out.println("indexCounter: "+ indexCounter);
      positionList.flush();
      positionIndex += 2; //For the new line and comma
    ...
    current = in.split(",");
    Integer counter = 0;
    String[] previous = {"z","z","z"};
    //System.out.println("previous: "+ previous[0]);

    //System.out.println("current: "+hashWord(current[0])+" previous: "+ hashWord(previous[0]));
    //boolean foo = hashWord(current[0])!=hashWord(previous[0]);
    //System.out.println("test: "+foo);
    boolean go = true;
    System.out.println("String Counter: "+ String.valueOf(counter));
    while(go){
      //System.out.println("word: "+ current[0]);
      if(hashWord(current[0])!=hashWord(previous[0])){ //kollar om ord är annorlunda från den andra, isåfall lägg in i hashtabell
        System.out.println("current: "+ current[0]+" counter: "+counter);
